  ...
  'isAfter': {
    ...
    checkConfig: function(constraint) {
      if (isNaN(constraint) && _.isNull(validator.toDate(constraint))) {
        return 'Date must be specified as a JS timestamp (number of ms since epoch), a natively-parseable date string, or a JavaScript Date instance; instead got `' + util.inspect(constraint) + '`.';
      }
      return false;
    }
